  ...
  try {
    ...
    grid.innerHTML = releases.map(r => `
      <article class="release">
        <div class="cover ${AD_escape(r.cover)}"></div>
...
async function AD_renderArtists() {
  ...
  try {
    ...
    grid.innerHTML = artists.map(a => `
      <a class="artist-card" href="${AD_escape(a.slug)}.html">
        <div class="artist-photo ${AD_escape(a.photo)}"></div>
...
async function AD_renderArtistDetail(slug) {
  ...
  try {
    ...
    document.querySelector('.artist-detail-photo').classList.add(artist.photo);
    ...
    document.getElementById('release-strip').innerHTML = releases.map(r => `
      <a class="release-strip-item" href="index.html#releases">
        <div class="release-strip-cover ${AD_escape(r.cover)}"></div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  AD_renderJournal();
  AD_renderAbout();
  if (document.body.dataset.artist) {
    AD_renderArtistDetail(document.body.dataset.artist);
  }
});
